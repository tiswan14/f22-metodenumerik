function calculateP2() {
  ...
  steps.innerHTML += `<li><strong>Langkah-langkah pengerjaan:</strong><br>1. Menghitung L<sub>0</sub> = (${x} - ${x1})(${x} - ${x2}) / (${xo} - ${x1})(${xo} - ${x2}) = ${L0.toFixed(
    8
  )}</li>`;

  // Menghitung L1
  let L1 = ((x - xo) * (x - x2)) / ((x1 - xo) * (x1 - x2));
  steps.innerHTML += `<li>2. Menghitung L<sub>1</sub> = (${x} - ${xo})(${x} - ${x2}) / (${x1} - ${xo})(${x1} - ${x2}) = ${L1.toFixed(
    8
  )}</li>`;

  // Menghitung L2
  let L2 = ((x - xo) * (x - x1)) / ((x2 - xo) * (x2 - x1));
  steps.innerHTML += `<li>3. Menghitung L<sub>2</sub> = (${x} - ${xo})(${x} - ${x1}) / (${x2} - ${xo})(${x2} - ${x1}) = ${L2.toFixed(
    8
  )}</li>`;
  ...
  steps.innerHTML += `<li>4. Menghitung P<sub>2</sub>(x) = ${yo} * ${L0.toFixed(
    8
